...
test('jailPay в банкротство завершает партию, а не подвешивает ход', () => {
  const s = createGame({
    players: [
      { id: 'you', name: 'Ты', isBot: false },
  ...
  s.players[0].inJail = true
  s.players[0].jailTurns = 1
  s.players[0].cash = 50 // не хватит на залог 500, активов нет
  const r = applyAction(s, { type: 'jailPay', playerId: 'you' })
  assert.equal(r.error, undefined)
  assert.equal(r.state.players[0].bankrupt, true)
  assert.equal(r.state.status, 'finished')
  assert.equal(r.state.winnerId, 'bot1')
})

test('jailRoll третий раз в банкротство завершает партию', () => {
  // ищем seed, где первый бросок из тюрьмы не дубль (иначе игрок просто выходит)
  let done = false
  for (let seed = 1; seed < 80 && !done; seed++) {
    ...
    s.players[0].cash = 50
    const r = applyAction(s, { type: 'jailRoll', playerId: 'you' })
    const d = r.state.dice!
    if (d[0] === d[1]) continue // выпал дубль — не наш случай
    done = true
    assert.equal(r.state.players[0].bankrupt, true)
    assert.equal(r.state.status, 'finished')
    assert.equal(r.state.winnerId, 'bot1')
  }
  assert.equal(done, true, 'не нашли seed с не-дублем')
})
