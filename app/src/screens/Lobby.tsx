    ...
    return (
      <div className="lobby rise">
        ...
        <div className="code-card" style={{ textAlign: 'center' }}>
          ...
          <h2 style={{ color: 'var(--ink)', marginTop: 6 }}>{t('Подбираем соперников')}<span className="dots-anim" /></h2>
          <p style={{ color: 'var(--ink-soft)', fontWeight: 800, marginTop: 6 }}>{t('Игра начнётся через пару секунд')}</p>
        </div>
    ...
    <div className="lobby rise">
      ...
      {isHost ? (
        <button className="btn gold block lg" style={{ maxWidth: 420, marginTop: 8 }} disabled={busy} onClick={startRoom}>
          {busy ? t('Раскладываем…') : t('Начать игру 🎲')}
        </button>
      ) : (
        <p style={{ marginTop: 8, textAlign: 'center', color: 'var(--ink-soft)', fontWeight: 800 }}>{t('Ждём, пока хозяин начнёт…')}</p>
      )}
    </div>
  )
}
